...
def scan_lesson_cached_audios(title):
    ...
    text_mapping = {"0": "快", "-20": "正常", "-30": "慢", "-40": "好慢"}
    found_tracks = [] 
...
# ==========================================================
# 📡 核心升級：先問 API 宜家可以用邊隻模型，拒絕 Hardcode 盲撞
# ==========================================================
...
with tab1:
    ...
    if files:
        ...
        if st.button("🚀 啟動 Gemini 視覺大腦識別", key="ocr_btn_t1"):
            with st.spinner(f"🔮 Gemini 視覺中樞正以動態模型探測穿透中..."):
...
with tab3:
    ...
    speed_level = st.radio("請調較老師的聽寫語速：", ["快", "正常", "慢", "好慢"], index=1, horizontal=True)
    
    speed_mapping = {"快": ("0%", 0), "正常": ("-20%", -20), "慢": ("-30%", -30), "好慢": ("-40%", -40)}
    ...
    if current_text.strip():
        st.markdown(f"""<div style="background-color: white; padding: 15px; border-radius: 5px; border: 1px solid #ddd; color: black; font-size: 16px; line-height: 1.6; white-space: pre-wrap; font-weight: 500;">{current_text}</div>""", unsafe_allow_html=True)
